#!/usr/bin/env python3
"""Fetch human UniProt domains, map to rat protein, then to GRCr8 genomic coords."""
...
GENES = {  # gene -> (chrom, strand, rat_tx, human_uniprot)
    "TP53":  ("NC_086028.1", "+", "NM_030989.3", "P04637"),
    "KMT2D": ("NC_086025.1", "-", "XM_063262838.1", "O14686"),
    "EP300": ("NC_086025.1", "+", "XM_039080287.2", "Q09472"),
    "FAT1":  ("NC_086034.1", "-", "XM_063275777.1", "Q14517"),
    "DICER1":("NC_086024.1", "-", "NM_001427215.1", "Q9UPY3"),
}
...
def build_h2r_map(rat_seq, hum_seq):
    """Return dict human_pos(1-based) -> rat_pos(1-based) for aligned residues."""
    aln = aligner.align(rat_seq, hum_seq)[0]
    tb, qb = aln.aligned  # target=rat, query=human
    m = {}
...
def aa_to_genomic_map(cds_df, strand):
    """Return list nucs where nucs[i] = genomic coord of CDS nt i (translation order)."""
...
for gene, (chrom, strand, rat_tx, up) in GENES.items():
    ...
    for ft in u.get("features", []):
        ...
        # map human AA range -> rat AA range
        ra = h2r.get(a)
        rb = h2r.get(b)
        if ra is None or rb is None:
            # try nearest mapped within range
